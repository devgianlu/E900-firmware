...
<% support_invmatch("IPV6_SUPPORT", "1", "/*"); %>
                       	new Array(topmenu.ipv6setup,  "index_ipv6.asp",  "<% get_lang(); %>_help/IPv6_Setup.asp"),
<% support_invmatch("IPV6_SUPPORT", "1", "*/"); %>
...
<% support_invmatch("WIRELESS_SUPPORT", "1", "/*"); %>
		new Array(
			...
			<% support_invmatch("GUEST_NETWORK_SUPPORT", "1", "//"); %>new Array(newui.gacs,"Guest_Access.asp","<% get_lang(); %>_help/Guest_Access.asp"),
...
<% support_invmatch("STORAGE_SUPPORT", "1", "/*"); %>
 		new Array(
			new Array(FUNST.Disk,   "storage/Disk_Management.asp",   "<% get_lang(); %>_help/Disk.asp",bmenu.storage),
			new Array(FUNST.MS,   "storage/Media_Server.asp",      "<% get_lang(); %>_help/Media_Server.asp"),
			new Array(FUNST.FTP,  "storage/FTP_Server.asp",     "<% get_lang(); %>_help/FTP_Server.asp"),
			new Array(bmenu.admin,"storage/NAS_Administration.asp","<% get_lang(); %>_help/Storage_Administration.asp")
		),
<% support_invmatch("STORAGE_SUPPORT", "1", "*/"); %>
<% support_invmatch("CES_PARENTAL_CONTROL_SUPPORT", "1", "/*"); %>
		new Array
		(
			new Array(<% show_access_restrictions(); %>,"PC_Filter.asp","<% get_lang(); %>_help/<% get_help_page(); %>",bmenu.accrestriction)
		),
<% support_invmatch("CES_PARENTAL_CONTROL_SUPPORT", "1", "*/"); %>
		new Array
		(
...
<% support_elsematch("IPV6_SUPPORT", "1",",","/*"); %>
                       	new Array(apptopmenu.ipv6_firewall, "IPv6_Firewall.asp", "<% get_lang(); %>_help/IPv6_Firewall.asp")
<% support_invmatch("IPV6_SUPPORT", "1", "*/"); %>
